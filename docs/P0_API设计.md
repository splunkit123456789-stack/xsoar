...
| 方法 | 路径 | 用途 | 所需权限 |
|---|---|---|---|
| `GET` | `/api/v1/soar/users` | 查询用户列表（分页） | `user:read` |
| `POST` | `/api/v1/soar/users` | 新增用户 | `user:manage` |
| `GET` | `/api/v1/soar/users/{id}` | 查询用户详情 | `user:read` |
| `PUT` | `/api/v1/soar/users/{id}` | 修改用户 | `user:manage` |
| `DELETE` | `/api/v1/soar/users/{id}` | 删除用户 | `user:manage` |
| `GET` | `/api/v1/soar/roles` | 查询角色列表 | `role:read` |
| `POST` | `/api/v1/soar/roles` | 新增角色 | `role:manage` |
| `GET` | `/api/v1/soar/roles/{id}` | 查询角色详情（含权限点） | `role:read` |
| `PUT` | `/api/v1/soar/roles/{id}` | 修改角色权限 | `role:manage` |
| `DELETE` | `/api/v1/soar/roles/{id}` | 删除角色 | `role:manage` |
...
- 所有受保护 API 必须经过统一鉴权中间件。
- 鉴权中间件从 Token 解析用户 ID，查询用户角色及权限点列表。
- 将权限点列表注入请求上下文（`ctx.Set("permissions", ...)`），Handler 中无需重复查询。
- 未授权访问统一返回：
